...
| ARROW-AUDIT-026 | Flight RPC protocol surface | Support Flight service methods, descriptors, IPC payloads, errors, and auth metadata. | Partial. Generated protocol, server core, gRPCServer listener path, DoGet/DoPut/DoExchange helpers, `CancelFlightInfo` action/result payload helpers, and Python-client smoke coverage including `PollFlightInfo` plus `CancelFlightInfo` `DoAction` exist. | No package-owned Julia Flight client runtime; handshake token propagation is still a local/server-core proof rather than external client-runtime coverage. | `src/flight/`, `test/flight.jl`, `test/flight_purehttp2.jl`, `test/flight_grpcserver.jl`, `test/flight/live_service_support.jl`, `README.md`. | P1 | Add external handshake-token proof and decide the Julia client-runtime boundary. |
...
6. Interop interface closure:
   ...
   owner exists.
7. Flight closure:
   add external handshake-token proof and decide whether Arrow.jl should
   include a package-owned Julia Flight client runtime.
