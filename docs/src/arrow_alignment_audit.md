...
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `runendencoded`, month-day-nano intervals, binary/fixed-size-binary hex JSON values, schema/field metadata, canonical extension storage+metadata fixtures, and physical JSON-to-IPC preservation for view/list-view/REE layouts. | Remaining gap is upstream Archery CLI parity for cross-language integration invocation and validation workflow parity. | `test/integrationtest.jl`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/run-end-encoded.json`, `test/arrowjson/canonical-extensions.json`, `src/arraytypes/views.jl`, `src/write.jl`, `src/metadata/Message.jl`. | P1 | Decide the upstream Archery parity boundary and whether to expose a repo-owned wrapper around it. |
...
| 2 | ARROW-AUDIT-021 | Integration coverage still needs an upstream Archery parity decision. |
