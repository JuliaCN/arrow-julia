...
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `fixedsizelist`, `runendencoded`, month-day-nano intervals, binary/fixed-size-binary hex JSON values, schema/field metadata, canonical extension storage+metadata fixtures, physical JSON-to-IPC preservation for view/list-view/fixed-size-list/REE layouts, an Archery-style IPC JSON CLI entry point with `--integration`, `--arrow`, `--json`, and `--mode`, plus package-local IPC file-to-stream and stream-to-file converter modes. A tested Archery adapter surface now exists under `dev/archery/`, including an executable wrapper, `JuliaTester` template, and Apache Arrow monorepo registration patch artifact for `--with-julia` / `ARCHERY_INTEGRATION_WITH_JULIA`. A focused subprocess smoke now runs the integration executable through JSON-to-Arrow, validation, Arrow-to-JSON, file-to-stream, and stream-to-file modes for baseline, modern physical-layout, and canonical extension fixtures, and CI runs the adapter, patch-artifact, and integration smoke as a dedicated job. The patch artifact applies to Apache Arrow checkout `603eeec8f6d75fa3d029be0aefbf1405a0dde69b`, the patched `tester_julia.py` compiles there, and patched upstream Archery IPC runner receipts now cover the `primitive`, `dictionary`, and `nested` selectors with Julia producing and consuming IPC data. The upstream receipts covered primitive, zero-length primitive, no-batch primitive, dictionary, unsigned dictionary, nested dictionary, nested, recursive nested, and nested large-offset generated JSON cases with `0 failures, 0 skips`. | Remaining gap is broader upstream Archery suite coverage beyond the focused primitive/dictionary/nested selectors and any upstream acceptance/CI decision in the Apache Arrow monorepo. | `dev/archery/`, `dev/archery/apache-arrow-archery-julia-registration.patch`, `test/archery_adapter_smoke.jl`, `test/archery_upstream_patch_smoke.jl`, `test/integrationtest.jl`, `test/integration_cli_smoke.jl`, `.github/workflows/ci.yml`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/nested.json`, `test/arrowjson/run-end-encoded.json`, `test/arrowjson/canonical-extensions.json`, `test/arrowjson_support/physical_columns.jl`, `src/arraytypes/fixedsizelist.jl`, `src/arraytypes/views.jl`, `src/write.jl`, `src/metadata/Message.jl`. | P1 | Expand the upstream Archery runner to the next bounded selectors only when Arrow.jl chooses to carry broader upstream suite parity, or keep the CI-guarded package-local adapter plus primitive/dictionary/nested upstream receipts as the integration boundary. |
...
2. Format and integration closure:
   ...
   runner also passes the focused `primitive`, `dictionary`, and `nested`
   selectors with Julia producing and consuming IPC data. Broader upstream
