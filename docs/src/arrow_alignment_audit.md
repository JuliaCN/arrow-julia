...
| ARROW-AUDIT-005 | BinaryView and Utf8View plus variadic buffers | Support view records and external variadic data buffers, including C Data's extra buffer-lengths buffer. | Covered for Arrow.jl read paths, writer support, C Data export/import, and integration JSON parsing of inline plus external view data. | Full upstream Archery parity and physical-layout-preserving JSON-to-IPC output are still separate integration decisions. | `src/arraytypes/views.jl`, `src/table.jl`, `src/cdata/export.jl`, `src/cdata/import.jl`, `test/runtests.jl`, `test/arrowjson/modern-layouts.json`, `test/cdata/export_tests.jl`. | P1 | Keep view-layout fixtures active and decide whether integration JSON should preserve physical view layout on write. |
...
| ARROW-AUDIT-007 | ListView and LargeListView | Support offsets, sizes, and child arrays for out-of-order or repeated spans. | Partial. Arrow.jl has metadata mapping, a native `Arrow.ListView`, C Data export/import coverage, and integration JSON parser coverage for list-view spans. | Full package-level IPC read/write fixture coverage and public producer ergonomics need a dedicated roundtrip test slice. | `src/arraytypes/list.jl`, `src/table.jl`, `src/cdata/export.jl`, `src/cdata/import.jl`, `test/arrowjson/modern-layouts.json`, `test/cdata/export_tests.jl`. | P1 | Add package-level IPC roundtrip tests for `ListView` / `LargeListView`, then move this row to Covered if writer evidence is complete. |
...
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `runendencoded`, month-day-nano intervals, and binary/fixed-size-binary hex JSON values. | Remaining gaps are upstream Archery CLI parity, physical-layout-preserving JSON-to-IPC output for view/list-view/REE layouts, and canonical extension JSON fixtures. | `test/integrationtest.jl`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/run-end-encoded.json`. | P1 | Add canonical extension fixtures and decide the Archery parity / physical-layout preservation boundary. |
...
2. Format and integration closure:
   ...
   `LargeListView`, REE read behavior, and month-day-nano intervals active;
...
| Order | ID | Why |
| --- | --- | --- |
| 1 | ARROW-AUDIT-007 | ListView has official layout status and now has integration JSON evidence, but still needs package-level IPC producer evidence. |
| 2 | ARROW-AUDIT-010 | REE is visible to users as read-only; write support or policy must be explicit. |
| 3 | ARROW-AUDIT-020 | Security validation is the highest-risk surface before expanding interop claims. |
| 4 | ARROW-AUDIT-021 | Integration coverage still needs canonical extension fixtures and an Archery parity decision. |
