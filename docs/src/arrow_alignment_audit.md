...
| ARROW-AUDIT-027 | Flight listener backends and transport performance | Package a clear live listener backend and performance evidence for large data movement. | Partial. `gRPCServer.jl` is the canonical packaged listener; nghttp2 is a bounded weakdep backend; large DoGet, large DoPut, bounded same-client reused DoPut, large DoExchange, concurrent DoGet, connection-isolated concurrent DoPut, and concurrent DoExchange performance receipts exist for the canonical listener path. | Backend parity remains incomplete: nghttp2 still lacks request-streaming Flight methods, and longer same-client upload-reuse soak remains an open production hardening gate. | `src/flight/server/backend_profiles.jl`, `test/flight_purehttp2_perf.jl`, `test/flight_nghttp2.jl`, `test/flight_grpcserver.jl`. | P2 | Extend backend parity and longer upload-reuse soak after the canonical gRPCServer path remains stable in CI. |
...
| ARROW-AUDIT-030 | Performance and zero-copy governance | Back zero-copy and performance claims with repeatable receipts. | Partial. C Data has allocation and pointer-identity receipts; IPC has stream/file write, metadata-read, and scan receipts; Flight has large DoGet, large DoPut, bounded same-client reused DoPut, large DoExchange, concurrent DoGet, connection-isolated concurrent DoPut, and concurrent DoExchange performance receipts. | IPC and Flight timing remain informational by default rather than strict wall-clock gates; longer same-client Flight upload-reuse soak is not yet a strict gate. | `test/cdata_validation_report.jl`, `test/ipc_performance_report.jl`, `test/flight_purehttp2_perf.jl`, CI workflow entries. | P2 | Decide whether selected performance receipts should graduate from informational metrics to strict local or CI gates. |
...
| 3 | ARROW-AUDIT-030 | Strict performance gates and longer same-client Flight upload-reuse soak remain the next production evidence policy gap after adding the bounded reuse receipt. |
