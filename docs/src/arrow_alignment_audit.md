...
| ARROW-AUDIT-027 | Flight listener backends and transport performance | Package a clear live listener backend and performance evidence for large data movement. | Partial. `gRPCServer.jl` is the canonical packaged listener; nghttp2 is a bounded weakdep backend; large DoGet, large DoPut, large DoExchange, and concurrent DoGet/DoExchange performance receipts exist for the canonical listener path. | Backend parity remains incomplete: nghttp2 still lacks request-streaming Flight methods, and concurrent/reused DoPut upload soak receipts are not yet tracked. | `src/flight/server/backend_profiles.jl`, `test/flight_purehttp2_perf.jl`, `test/flight_nghttp2.jl`, `test/flight_grpcserver.jl`. | P2 | Extend backend parity and concurrent upload performance receipts after the canonical gRPCServer path remains stable in CI. |
...
| ARROW-AUDIT-030 | Performance and zero-copy governance | Back zero-copy and performance claims with repeatable receipts. | Partial. C Data has allocation and pointer-identity receipts; Flight has large DoGet, large DoPut, large DoExchange, and concurrent DoGet/DoExchange performance receipts. | IPC read/write and concurrent/reused Flight upload performance do not yet have the same audit-gated baseline. | `test/cdata_validation_report.jl`, `test/flight_purehttp2_perf.jl`, CI workflow entries. | P2 | Add small, stable IPC read/write performance receipts before broadening performance claims. |
