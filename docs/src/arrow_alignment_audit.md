...
| --- | --- | --- | --- | --- | --- |
| 1 | Official integration testing | `ARROW-AUDIT-021` | Keep the package-local Archery-style CLI, tested `dev/archery/` adapter, Apache Arrow monorepo registration patch artifact, and current upstream apply/compile receipt as the executable contract, now guarded by subprocess smoke coverage and a focused CI job. | Run the patched Apache Arrow Archery integration command against this package and capture the upstream runner receipt, or keep the CI-guarded package-local adapter as the integration boundary. | Integration CLI smoke tests, JSON fixture parity, adapter smoke tests, patch-artifact smoke tests, upstream apply/compile receipts, and any upstream runner receipt if adopted. |
...
2. Format and integration closure:
   ...
   `603eeec8f6d75fa3d029be0aefbf1405a0dde69b`; next run the patched upstream
   Archery command if this package should participate in upstream
   cross-language runs.
