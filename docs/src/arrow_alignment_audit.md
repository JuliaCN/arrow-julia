...
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `runendencoded`, month-day-nano intervals, binary/fixed-size-binary hex JSON values, schema/field metadata, canonical extension storage+metadata fixtures, physical JSON-to-IPC preservation for view/list-view/REE layouts, and an Archery-style IPC JSON CLI entry point with `--integration`, `--arrow`, `--json`, and `--mode`. | Remaining gaps are upstream Archery tester registration and dedicated stream/file converter wrapper parity for the full IPC runner workflow. | `test/integrationtest.jl`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/run-end-encoded.json`, `test/arrowjson/canonical-extensions.json`, `src/arraytypes/views.jl`, `src/write.jl`, `src/metadata/Message.jl`. | P1 | Decide whether to add package-local stream/file converter wrappers or pursue upstream Archery tester registration first. |
...
| 2 | ARROW-AUDIT-021 | Integration coverage still needs stream/file converter wrapper parity and an upstream Archery registration decision. |
