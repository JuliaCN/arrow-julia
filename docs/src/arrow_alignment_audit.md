...
| ARROW-AUDIT-026 | Flight RPC protocol surface | Support Flight service methods, descriptors, IPC payloads, errors, and auth metadata. | Partial. Generated protocol, server core, gRPCServer listener path, DoGet/DoPut/DoExchange helpers, `CancelFlightInfo` action/result payload helpers, and Python-client smoke coverage including `Handshake` token propagation, `PollFlightInfo`, and `CancelFlightInfo` `DoAction` exist. | No package-owned Julia Flight client runtime exists yet; external interop proof is currently owned by Python clients generated from the package `Flight.proto` or by PyArrow. | `src/flight/`, `test/flight.jl`, `test/flight_purehttp2.jl`, `test/flight_grpcserver.jl`, `test/flight/live_service_support.jl`, `README.md`. | P1 | Decide the Julia client-runtime boundary. |
...
6. Interop interface closure:
   ...
   owner exists.
7. Flight closure:
   decide whether Arrow.jl should include a package-owned Julia Flight client
   runtime.
...
| 3 | ARROW-AUDIT-026 | Flight has substantial implementation and external control-plane proof; the remaining decision is the Julia client-runtime boundary. |
