...
| ARROW-AUDIT-026 | Flight RPC protocol surface | Support Flight service methods, descriptors, IPC payloads, errors, and auth metadata. | Partial. Generated protocol, server core, gRPCServer listener path, DoGet/DoPut/DoExchange helpers, `CancelFlightInfo` action/result payload helpers, and Python-client smoke coverage exist. | No package-owned Julia Flight client runtime; `CancelFlightInfo`, `PollFlightInfo`, and handshake token propagation are still local/server-core proofs rather than external client-runtime coverage. | `src/flight/`, `test/flight.jl`, `test/flight_purehttp2.jl`, `test/flight_grpcserver.jl`, `README.md`. | P1 | Add external `CancelFlightInfo`, `PollFlightInfo`, and handshake-token proofs, and decide the Julia client-runtime boundary. |
...
6. Interop interface closure:
   ...
   owner exists.
7. Flight closure:
   add external `CancelFlightInfo`, `PollFlightInfo`, and handshake-token
   proofs, and decide whether Arrow.jl should include a package-owned Julia
   Flight client runtime.
