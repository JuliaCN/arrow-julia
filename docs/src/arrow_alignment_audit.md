...
| --- | --- | --- | --- | --- | --- |
| 1 | Official integration testing | `ARROW-AUDIT-021` | Keep the package-local Archery-style CLI, tested `dev/archery/` adapter, Apache Arrow monorepo registration patch artifact, current upstream apply/compile receipt, and focused upstream primitive runner receipt as the executable contract, now guarded by subprocess smoke coverage and a focused CI job. | Expand the patched Apache Arrow Archery runner selector beyond `primitive` if Arrow.jl should pursue broader upstream suite parity, or keep the CI-guarded package-local adapter plus focused upstream receipt as the integration boundary. | Integration CLI smoke tests, JSON fixture parity, adapter smoke tests, patch-artifact smoke tests, upstream apply/compile receipts, and upstream runner receipts for any adopted selector. |
...
2. Format and integration closure:
   ...
   runner also passes the focused `primitive` selector with Julia producing
   and consuming IPC data. Broader upstream suite coverage is a separate
   adoption decision.
