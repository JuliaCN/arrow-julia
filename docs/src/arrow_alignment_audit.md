...
| ARROW-AUDIT-023 | C Stream Interface | Expose `ArrowArrayStream` callbacks for same-process pull-style streaming. | Partial. Arrow.jl exports a single `Arrow.Table` as one C Stream batch through `Arrow.CData.exportstream` / `exportstream!`, using existing C Data schemas, arrays, and release callbacks. | C Stream import is not implemented yet, and multi-batch producer ownership remains a follow-up design. | `src/cdata/stream.jl`, `include/arrow_julia_cdata.h`, `test/cdata/stream_tests.jl`, `test/cdata_smoke.c`, `docs/src/cdata_alignment.md`. | P2 | Add C Stream import or explicitly accept producer-only support as the package boundary, then decide PyCapsule on top of that ownership model. |
...
6. Interop interface closure:
   close the C Stream import or producer-only boundary decision, then decide
   PyCapsule on top of the accepted stream and array ownership model; keep C
   Device deferred until a device-memory owner exists.
7. Flight closure:
