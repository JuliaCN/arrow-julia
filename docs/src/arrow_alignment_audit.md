...
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `runendencoded`, month-day-nano intervals, binary/fixed-size-binary hex JSON values, schema/field metadata, canonical extension storage+metadata fixtures, physical JSON-to-IPC preservation for view/list-view/REE layouts, an Archery-style IPC JSON CLI entry point with `--integration`, `--arrow`, `--json`, and `--mode`, plus package-local IPC file-to-stream and stream-to-file converter modes. A tested Archery adapter surface now exists under `dev/archery/`, including an executable wrapper, `JuliaTester` template, and Apache Arrow monorepo registration patch artifact for `--with-julia` / `ARCHERY_INTEGRATION_WITH_JULIA`. A focused subprocess smoke now runs the integration executable through JSON-to-Arrow, validation, Arrow-to-JSON, file-to-stream, and stream-to-file modes for baseline, modern physical-layout, and canonical extension fixtures, and CI runs the adapter, patch-artifact, and integration smoke as a dedicated job. | Remaining gap is applying and validating the registration patch inside the Apache Arrow monorepo for automated cross-language runner participation. | `dev/archery/`, `dev/archery/apache-arrow-archery-julia-registration.patch`, `test/archery_adapter_smoke.jl`, `test/archery_upstream_patch_smoke.jl`, `test/integrationtest.jl`, `test/integration_cli_smoke.jl`, `.github/workflows/ci.yml`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/run-end-encoded.json`, `test/arrowjson/canonical-extensions.json`, `src/arraytypes/views.jl`, `src/write.jl`, `src/metadata/Message.jl`. | P1 | Apply the upstream Apache Arrow Archery registration patch in an Apache Arrow checkout and capture an upstream runner receipt, or keep the CI-guarded package-local adapter as the integration boundary. |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Official integration testing | `ARROW-AUDIT-021` | Keep the package-local Archery-style CLI, tested `dev/archery/` adapter, and Apache Arrow monorepo registration patch artifact as the current executable contract, now guarded by subprocess smoke coverage and a focused CI job. | Apply the upstream Apache Arrow Archery registration patch in an Apache Arrow checkout and capture an upstream runner receipt, or keep the CI-guarded package-local adapter as the integration boundary. | Integration CLI smoke tests, JSON fixture parity, adapter smoke tests, patch-artifact smoke tests, and any upstream runner receipt if adopted. |
...
2. Format and integration closure:
   ...
   canonical extension metadata active; next apply and validate the upstream
   Archery tester registration patch in an Apache Arrow checkout if this
   package should participate in upstream cross-language runs.
