...
| ARROW-AUDIT-020 | Security and robustness guidance | Reject malformed layouts safely, validate dangerous metadata, and keep untrusted IPC/C Data risks documented. | Partial. IPC List/Binary/Utf8/Map/ListView offset validation, canonical extension negative tests, C Data import validation, and public security guidance are covered; no broad fuzz/regression corpus or public validator API exists yet. | Untrusted IPC and C Data surfaces still need larger malformed-file coverage before broader security claims. | `src/table.jl`, `src/arraytypes/list.jl`, `src/cdata/import.jl`, `test/runtests.jl`, `test/cdata/base_contract_tests.jl`, `test/cdata/import_*_tests.jl`, `docs/src/security.md`. | P1 | Extend the regression corpus for invalid UTF-8, extension metadata combinations, and C Data soundness; then decide whether to expose a public validator API. |
