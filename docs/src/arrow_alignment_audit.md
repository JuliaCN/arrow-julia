...
| ARROW-AUDIT-003 | Decimal, date/time, timestamp, duration, and interval types | Preserve Arrow logical scalar storage, including decimal widths and interval units. | Covered for package runtime and C Data, including month-day-nano interval storage. | Integration JSON fixture coverage still lags newer interval variants; tracked separately in `ARROW-AUDIT-021`. | `src/eltypes.jl`, `src/cdata/formats.jl`, `test/runtests.jl`, `test/cdata/import_nested_tests.jl`. | P1 | Add integration JSON fixtures for month-day-nano interval in the integration-test closure slice. |
...
| ARROW-AUDIT-005 | BinaryView and Utf8View plus variadic buffers | Support view records and external variadic data buffers, including C Data's extra buffer-lengths buffer. | Covered for Arrow.jl read paths, writer support, and C Data export/import. | Upstream-style integration JSON coverage for view layouts is still thin. | `src/arraytypes/views.jl`, `src/table.jl`, `src/cdata/export.jl`, `src/cdata/import.jl`, `test/runtests.jl`, `test/cdata/export_tests.jl`. | P1 | Add view-layout fixtures to the integration harness and keep C Data buffer-length tests. |
...
| ARROW-AUDIT-007 | ListView and LargeListView | Support offsets, sizes, and child arrays for out-of-order or repeated spans. | Partial. Arrow.jl has metadata mapping, a native `Arrow.ListView`, and C Data export/import coverage. | Full IPC read/write fixture coverage and public producer ergonomics need a dedicated roundtrip test slice. | `src/arraytypes/list.jl`, `src/table.jl`, `src/cdata/export.jl`, `src/cdata/import.jl`, `test/cdata/export_tests.jl`. | P1 | Add package-level IPC roundtrip tests for `ListView` / `LargeListView`, then move this row to Covered if writer evidence is complete. |
...
| ARROW-AUDIT-010 | Run-End Encoded layout | Read REE arrays and preserve the `run_ends` / `values` child contract. | Partial. Read path and C Data export/import are covered; IPC write paths reject REE. | Full support remains incomplete until a writer representation and roundtrip policy are accepted. | `src/arraytypes/runendencoded.jl`, `src/table.jl`, `test/run_end_encoded_small.arrow`, `test/runtests.jl`, `test/cdata/import_nested_tests.jl`. | P1 | Decide whether to implement REE writing or keep explicit read-only support as the documented policy. |
...
| ARROW-AUDIT-020 | Security and robustness guidance | Reject malformed layouts safely, validate dangerous metadata, and keep untrusted IPC/C Data risks documented. | Partial. Many structural checks exist, and C Data import has targeted negative validation; no public full-array validator or fuzz/regression gate is documented. | Untrusted IPC and C Data surfaces need clearer validation guidance before broader security claims. | `src/table.jl`, `src/cdata/import.jl`, `test/runtests.jl`, `test/cdata/base_contract_tests.jl`, `test/cdata/import_*_tests.jl`. | P1 | Add a security validation plan: regression files, malformed offsets, invalid UTF-8, malformed extension metadata, and C Data soundness checks. |
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists, but fixture coverage lags newer layouts and canonical extensions. | Modern layouts such as view types, list-view, REE, month-day-nano intervals, and canonical extension cases need fixture coverage. | `test/integrationtest.jl`, `test/arrowjson.jl`, `test/arrowjson/`. | P1 | Add fixture rows before claiming full cross-language integration coverage for newer formats. |
...
| ARROW-AUDIT-026 | Flight RPC protocol surface | Support Flight service methods, descriptors, IPC payloads, errors, and auth metadata. | Partial. Generated protocol, server core, gRPCServer listener path, DoGet/DoPut/DoExchange helpers, and Python-client smoke coverage exist. | No package-owned Julia Flight client runtime; `CancelFlightInfo` is still only proto data/action payload material, `PollFlightInfo` and handshake token propagation are local/server-core proofs in current tests. | `src/flight/`, `test/flight.jl`, `test/flight_purehttp2.jl`, `test/flight_grpcserver.jl`, `README.md`. | P1 | Add `CancelFlightInfo` action helpers, external poll/handshake proofs, and decide the Julia client-runtime boundary. |
| ARROW-AUDIT-027 | Flight listener backends and transport performance | Package a clear live listener backend and performance evidence for large data movement. | Partial. `gRPCServer.jl` is the canonical packaged listener; nghttp2 is a bounded weakdep backend; large DoGet performance runner exists. | Request-streaming performance for DoPut/DoExchange and backend parity remain incomplete. | `src/flight/server/backend_profiles.jl`, `test/flight_purehttp2_perf.jl`, `test/flight_nghttp2.jl`, `test/flight_grpcserver.jl`. | P2 | Extend performance receipts after Flight method-completeness gaps are closed. |
...
| ARROW-AUDIT-030 | Performance and zero-copy governance | Back zero-copy and performance claims with repeatable receipts. | Partial. C Data has allocation and pointer-identity receipts; Flight has large DoGet performance receipts. | IPC read/write and broader Flight method performance do not yet have the same audit-gated baseline. | `test/cdata_validation_report.jl`, `test/flight_purehttp2_perf.jl`, CI workflow entries. | P2 | Add small, stable IPC read/write performance receipts before broadening performance claims. |
...
2. Format and integration closure:
   add integration fixtures for `Utf8View` / `BinaryView`, `ListView` /
   `LargeListView`, REE read behavior, month-day-nano intervals, and canonical
   extensions that are currently only covered by unit tests.
...
5. Robustness closure:
   add a validation and regression-file plan for untrusted IPC, malformed
   offsets, malformed extension metadata, and C Data soundness checks.
6. Interop interface closure:
   ...
   owner exists.
7. Flight closure:
   add `CancelFlightInfo` action helpers, external `PollFlightInfo` and
   handshake-token proofs, and decide whether Arrow.jl should include a
   package-owned Julia Flight client runtime.
...
| Order | ID | Why |
| --- | --- | --- |
| 1 | ARROW-AUDIT-021 | Integration coverage is the broadest proof gap for newer official layouts. |
| 2 | ARROW-AUDIT-007 | ListView has official layout status but needs package-level IPC evidence. |
| 3 | ARROW-AUDIT-010 | REE is visible to users as read-only; write support or policy must be explicit. |
| 4 | ARROW-AUDIT-020 | Security validation is the highest-risk surface before expanding interop claims. |
| 5 | ARROW-AUDIT-026 | Flight has substantial implementation but needs cancellation, poll, handshake, and client-boundary closure. |
