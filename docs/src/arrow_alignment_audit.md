...
| ARROW-AUDIT-008 | Struct, Map, DenseUnion, SparseUnion | Preserve nested fields, map entries, union type ids, and dense offsets. | Covered for normal package use, C Data export/import, and integration JSON Map entries whose child names are non-canonical but positionally valid. | Large-offset maps remain rejected in C Data because the C Data map format has no offset-width variant. | `src/arraytypes/struct.jl`, `src/arraytypes/map.jl`, `src/arraytypes/unions.jl`, `src/table/build_nested.jl`, `src/cdata/export.jl`, `docs/src/cdata_alignment.md`, `test/arrowjson/map-non-canonical.json`. | P0 | Keep explicit rejection for C Data large-offset maps and keep malformed union plus non-canonical Map fixtures active. |
...
| ARROW-AUDIT-020 | Security and robustness guidance | Reject malformed layouts safely, validate dangerous metadata, and keep untrusted IPC/C Data risks documented. | Partial. IPC FieldNode length/null-count validation, IPC List/Binary/Utf8/Map/ListView offset validation, IPC BinaryView/Utf8View span validation, IPC Utf8/LargeUtf8/Utf8View byte validation, IPC dictionary index validation, IPC DenseUnion type-id/offset validation, IPC SparseUnion type-id and child-length validation, canonical extension metadata/storage negative tests, C Data released input validation, C Data format-string validation, C Data metadata byte validation, C Data base-shape count validation, C Data child pointer and dictionary peer validation, C Data Binary/UTF-8 offset validation, C Data UTF-8/UTF-8 view byte validation, C Data dictionary index validation, C Data ListView negative offset/size/span validation, C Data Map offset validation, C Data DenseUnion type-id/offset validation, C Data SparseUnion child-length validation, C Data Run-End Encoded run-end shape validation, and public security guidance are covered; no broad fuzz/regression corpus or public validator API exists yet. | Untrusted IPC and C Data surfaces still need larger malformed-file coverage before broader security claims. | `src/table.jl`, `src/table/build_common.jl`, `src/table/build_nested.jl`, `src/arraytypes/list.jl`, `src/arraytypes/views.jl`, `src/arraytypes/dictencoding.jl`, `src/arraytypes/unions.jl`, `src/eltypes.jl`, `src/logicaltypes_builtin.jl`, `src/cdata/import.jl`, `src/cdata/import/`, `test/runtests.jl`, `test/runtests/misc_validation_views.jl`, `test/runtests/misc_validation_nodes.jl`, `test/runtests/misc_validation_unions.jl`, `test/cdata/base_contract_tests.jl`, `test/cdata/import_*_tests.jl`, `docs/src/security.md`. | P1 | Extend malformed IPC/C Data coverage to any remaining nested edge cases and future official extension combinations; then decide whether to expose a public validator API. |
| ARROW-AUDIT-021 | Official integration testing | Provide JSON-to-Arrow, Arrow-to-JSON, and validation entry points that can be composed with cross-language integration tests. | Partial. The harness exists and now recognizes `utf8view`, `binaryview`, `listview`, `largelistview`, `fixedsizelist`, `runendencoded`, month-day-nano intervals, binary/fixed-size-binary hex JSON values, schema/field metadata, canonical extension storage+metadata fixtures, physical JSON-to-IPC preservation for view/list-view/fixed-size-list/REE layouts, an Archery-style IPC JSON CLI entry point with `--integration`, `--arrow`, `--json`, and `--mode`, plus package-local IPC file-to-stream and stream-to-file converter modes. A tested Archery adapter surface now exists under `dev/archery/`, including an executable wrapper, `JuliaTester` template, and Apache Arrow monorepo registration patch artifact for `--with-julia` / `ARCHERY_INTEGRATION_WITH_JULIA`. A focused subprocess smoke now runs the integration executable through JSON-to-Arrow, validation, Arrow-to-JSON, file-to-stream, and stream-to-file modes for baseline, modern physical-layout, canonical extension, nested, and non-canonical Map fixtures, and CI runs the adapter, patch-artifact, and integration smoke as a dedicated job. The patch artifact applies to Apache Arrow checkout `603eeec8f6d75fa3d029be0aefbf1405a0dde69b`, the patched `tester_julia.py` compiles there, and patched upstream Archery IPC runner receipts now cover the `primitive`, `dictionary`, `nested`, and `map` selectors with Julia producing and consuming IPC data. The upstream receipts covered primitive, zero-length primitive, no-batch primitive, dictionary, unsigned dictionary, nested dictionary, nested, recursive nested, nested large-offset, canonical map, and non-canonical map generated JSON cases with `0 failures, 0 skips`. | Remaining gap is broader upstream Archery suite coverage beyond the focused primitive/dictionary/nested/map selectors and any upstream acceptance/CI decision in the Apache Arrow monorepo. | `dev/archery/`, `dev/archery/apache-arrow-archery-julia-registration.patch`, `test/archery_adapter_smoke.jl`, `test/archery_upstream_patch_smoke.jl`, `test/integrationtest.jl`, `test/integration_cli_smoke.jl`, `.github/workflows/ci.yml`, `test/arrowjson.jl`, `test/arrowjson/modern-layouts.json`, `test/arrowjson/nested.json`, `test/arrowjson/map-non-canonical.json`, `test/arrowjson/run-end-encoded.json`, `test/arrowjson/canonical-extensions.json`, `test/arrowjson_support/arrow_array.jl`, `test/arrowjson_support/physical_columns.jl`, `src/arraytypes/fixedsizelist.jl`, `src/arraytypes/map.jl`, `src/arraytypes/views.jl`, `src/write.jl`, `src/metadata/Message.jl`. | P1 | Expand the upstream Archery runner to the next bounded selectors only when Arrow.jl chooses to carry broader upstream suite parity, or keep the CI-guarded package-local adapter plus primitive/dictionary/nested/map upstream receipts as the integration boundary. |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Official integration testing | `ARROW-AUDIT-021` | Keep the package-local Archery-style CLI, tested `dev/archery/` adapter, Apache Arrow monorepo registration patch artifact, current upstream apply/compile receipt, and focused upstream primitive/dictionary/nested/map runner receipts as the executable contract, now guarded by subprocess smoke coverage and a focused CI job. | Expand the patched Apache Arrow Archery runner to the next bounded selectors if Arrow.jl should pursue broader upstream suite parity, or keep the CI-guarded package-local adapter plus focused upstream receipts as the integration boundary. | Integration CLI smoke tests, JSON fixture parity, adapter smoke tests, patch-artifact smoke tests, upstream apply/compile receipts, and upstream runner receipts for any adopted selector. |
| 2 | Robustness and validator surface | `ARROW-AUDIT-020` | Keep malformed-layout rejection inside readers/importers for now. | Decide whether to expose a public validation API or expand the private malformed corpus only. | Negative IPC and C Data fixtures plus focused reader/import tests for every newly covered shape. |
...
2. Format and integration closure:
   ...
   runner also passes the focused `primitive`, `dictionary`, `nested`, and
   `map` selectors with Julia producing and consuming IPC data. Broader upstream
   suite coverage is a separate adoption decision.
