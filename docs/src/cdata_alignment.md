...
| CDATA-AUDIT-013 | C Data import lacked focused malformed-offset, UTF-8 byte, and nested-shape regression coverage. | C Data import now rejects malformed base counts, rejects null child pointer entries, rejects missing dictionary schema/array peers, rejects negative or non-monotonic Binary/UTF-8 offsets, rejects invalid UTF-8 bytes for non-null UTF-8 and UTF-8 view values, keeps null-slot bytes ignored, rejects malformed dictionary indices, rejects malformed Map offsets, rejects DenseUnion undeclared type IDs and out-of-bounds offsets, rejects SparseUnion child-length mismatches, and rejects malformed Run-End Encoded run-end children. | `src/cdata/import.jl`, `src/cdata/import/`, `src/cdata/imported_vectors.jl`, `test/cdata/base_contract_tests.jl`, `test/cdata/import_malformed_tests.jl`, `test/cdata/import_basic_tests.jl`, `test/cdata/import_nested_tests.jl`. | P1 | Closed | Landed when malformed base, pointer, scalar, view, dictionary, map, union, and REE imports failed before exposing borrowed vectors, while valid zero-copy imports still passed. |
