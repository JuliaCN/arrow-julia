...
| CDATA-AUDIT-011 | Performance governance was a local receipt, not a CI gate. | `test/cdata_validation_report.jl` now supports configurable import allocation and timing limits, and CI runs it with `ARROW_CDATA_MAX_IMPORT_ALLOC_BYTES=5000000` while leaving wall-clock timing informational by default. | `test/cdata_validation_report.jl`, `.github/workflows/ci.yml`. | P2 | Closed | Landed when CI gated obvious import allocation regressions without default wall-clock failures. |
