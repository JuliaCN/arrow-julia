...
| C Stream Interface | `ArrowArrayStream` is a pull-style stream of schemas and arrays using callbacks. | `exportstream` and `exportstream!` export an `Arrow.Table` as one pull-style `ArrowArrayStream` batch, followed by a released end-of-stream array. | Not imported as C Stream. | `src/cdata/stream.jl`, `include/arrow_julia_cdata.h`, `test/cdata/stream_tests.jl`, `test/cdata_smoke.c`. | Partial | Add C Stream import or document producer-only support as the accepted package boundary. |
...
| CDATA-AUDIT-008 | C Stream was not part of the package-owned C interface surface. | `Arrow.CData.ArrowArrayStream`, `exportstream`, and `exportstream!` now expose a single-batch C Stream producer over the existing C Data exporter, with C smoke coverage for schema, batch, end-of-stream, and release behavior. | `src/cdata/stream.jl`, `include/arrow_julia_cdata.h`, `test/cdata/stream_tests.jl`, `test/cdata_smoke.c`. | P2 | Partial | Close when C Stream import is implemented or producer-only support is accepted as the package boundary. |
...
1. Close the remaining `CDATA-AUDIT-008` import decision before claiming full
   C Stream support.
