...
| Schema and field metadata bytes | `ArrowSchema.metadata` uses the official native-endian key/value byte layout; absent metadata can be `NULL`. Extension type names and extension metadata are encoded there. | Schema and field metadata are exported from `Arrow.getmetadata` and `Arrow.withmetadata`. | Imported tables borrow metadata-carrying schemas but do not expose high-level metadata accessors or reconstruct extension types. | Metadata export tests and `ImportedTable` fields. | Partial | `CDATA-AUDIT-001`, `CDATA-AUDIT-002`. |
...
| --- | --- | --- | --- | --- | --- | --- |
| CDATA-AUDIT-001 | Imported C Data metadata is borrowed but not exposed through a high-level `Arrow.getmetadata` surface on `ImportedTable` or imported columns. | `ImportedTable` stores base pointers, names, and columns only. | `src/cdata/imported_vectors.jl`, `src/cdata/import.jl`. | P1 | Open | Imported schema and field metadata can be read without copying data buffers, with tests for schema and column metadata. |
| CDATA-AUDIT-002 | Extension type reconstruction from C Data metadata is not implemented. | Manual and import code treat logical extension reconstruction as unsupported. | `src/cdata/import.jl`, ArrowTypes integration. | P1 | Open | C Data import recognizes extension metadata and either reconstructs supported extension types or exposes a documented raw-metadata fallback. |
