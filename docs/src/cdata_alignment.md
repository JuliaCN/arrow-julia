...
| CDATA-AUDIT-013 | Variable-width C Data import lacked focused malformed-offset and UTF-8 byte regression coverage. | C Data import now rejects negative or non-monotonic Binary/UTF-8 offsets, rejects invalid UTF-8 bytes for non-null UTF-8 and UTF-8 view values, and keeps null-slot bytes ignored. | `src/cdata/import.jl`, `test/cdata/import_basic_tests.jl`, `test/cdata/import_nested_tests.jl`. | P1 | Closed | Landed when malformed offset and malformed UTF-8 imports failed before exposing borrowed vectors, while valid zero-copy imports still passed. |
