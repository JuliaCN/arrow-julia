...
| --- | --- | --- | --- | --- | --- | --- |
| Base struct ABI | `ArrowSchema` and `ArrowArray` carry format, metadata, children, dictionary, buffers, release callbacks, and producer private data. | `Arrow.CData.ArrowSchema`, `Arrow.CData.ArrowArray`, `exporttable`, and `exporttable!`. | `importtable` borrows a top-level struct array and rejects impossible length, null-count, buffer-count, child-count, child-pointer, and dictionary-peer shapes through focused import submodules. | `src/cdata/core.jl`, `src/cdata/export.jl`, `src/cdata/import.jl`, `src/cdata/import/`, `test/cdata.jl`. | Covered | Keep ABI field order, modular import validation, and release behavior guarded by focused tests. |
...
| CDATA-AUDIT-013 | C Data import lacked focused malformed-offset, UTF-8 byte, metadata, and nested-shape regression coverage. | C Data import now rejects malformed metadata entry counts and string byte lengths, rejects malformed base counts, rejects null child pointer entries, rejects missing dictionary schema/array peers, rejects negative or non-monotonic Binary/UTF-8 offsets, rejects invalid UTF-8 bytes for non-null UTF-8 and UTF-8 view values, keeps null-slot bytes ignored, rejects malformed dictionary indices, rejects malformed Map offsets, rejects DenseUnion undeclared type IDs and out-of-bounds offsets, rejects SparseUnion child-length mismatches, and rejects malformed Run-End Encoded run-end children. | `src/cdata/import.jl`, `src/cdata/import/`, `src/cdata/imported_vectors.jl`, `test/cdata/base_contract_tests.jl`, `test/cdata/import_malformed_tests.jl`, `test/cdata/import_basic_tests.jl`, `test/cdata/import_nested_tests.jl`. | P1 | Closed | Landed when malformed metadata, base, pointer, scalar, view, dictionary, map, union, and REE imports failed before exposing borrowed vectors, while valid zero-copy imports still passed. |
