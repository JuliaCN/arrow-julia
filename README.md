...
Flight RPC status:
  ...
  * Includes transport-neutral client helpers for request headers, binary metadata, and URI parsing in the base package; the legacy gRPC-backed remote Flight client remains behind the optional `gRPCClient.jl` weakdep for downstream compatibility, while package-owned interop and performance proofs now use external Python clients
  * Keeps the Flight client compatibility layer modular under `src/flight/client/`, with the base shell retained in `src/flight/client.jl` and the legacy gRPC-backed runtime loaded through `ext/ArrowFlightgRPCClientExt.jl`
