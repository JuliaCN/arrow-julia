...
Flight RPC status:
  ...
  * The packaged Flight server backend contract now reports the built-in `:purehttp2` path as the only default live listener profile, exposes an optional weakdep-backed `:grpcserver` compatibility profile when `gRPCServer.jl` is loaded, and exposes a weakdep-backed `:nghttp2` profile only when `Nghttp2Wrapper.jl` is loaded; the grpcserver bridge reaches the latest `gRPCServer.jl` server stack while the nghttp2 backend is still limited to unary plus buffered server-streaming methods with trailer-borne `grpc-status`
  ...
  * Includes `test/flight_grpcserver.jl` as the focused weakdep-backed grpcserver compatibility runner; it revives `ArrowgRPCServerExt` against the latest `gRPCServer.jl` listener stack and proves descriptor bridging plus live Python-client Flight smoke over that optional backend
