...
Flight RPC status:
  ...
  * Includes `test/flight_purehttp2_perf.jl` as a focused large-transport runner that benchmarks large-response `DoGet` plus large request-streaming `DoPut` and `DoExchange` on the gRPCServer-owned listener path over the PureHTTP2 substrate through a reusable backend-factory seam, and also replays env-tunable concurrent `DoGet` and `DoExchange` soak rounds through `ARROW_FLIGHT_PYARROW_CONCURRENT_CLIENTS`, `ARROW_FLIGHT_PYARROW_REQUESTS_PER_CLIENT`, and `ARROW_FLIGHT_PYARROW_SOAK_ROUNDS`
