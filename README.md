...
Flight RPC status:
  ...
  * `CancelFlightInfo`, `Handshake` token propagation, and `PollFlightInfo` currently remain server-core/local proofs because the current external Python client surfaces used in tests do not cover those contracts directly
