...
Flight RPC status:
  ...
  * Includes package-owned live Python-client coverage for authenticated `ListFlights`, `GetFlightInfo`, `PollFlightInfo`, `GetSchema`, `DoGet`, `DoPut`, `DoExchange`, `ListActions`, and `DoAction`, including the official `CancelFlightInfo` action payload, through `test/flight_purehttp2.jl`
  ...
  * Includes `test/flight_purehttp2_perf.jl` as a focused large-transport runner that benchmarks large-response `DoGet` plus large request-streaming `DoPut` and `DoExchange` on the gRPCServer-owned listener path over the PureHTTP2 substrate through a reusable backend-factory seam, and also replays env-tunable concurrent `DoGet`, connection-isolated concurrent `DoPut`, and concurrent `DoExchange` soak rounds through `ARROW_FLIGHT_PYARROW_CONCURRENT_CLIENTS`, `ARROW_FLIGHT_PYARROW_REQUESTS_PER_CLIENT`, and `ARROW_FLIGHT_PYARROW_SOAK_ROUNDS`
