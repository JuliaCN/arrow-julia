...
const GRPCSERVER_COMPARE_URL = "https://github.com/tao3k/gRPCServer.jl.git"
const GRPCSERVER_COMPARE_REV = "1d577174be169ccc1cd708c9124716b181165e80"
...
function flight_transport_compare_roots(path::AbstractString)
    roots = String[]
...
function maybe_locate_purehttp2()
    ...
    for root in flight_transport_compare_roots(TEST_ROOT)
        for candidate in (
...
include(joinpath(TEST_ROOT, "flight", "transport_compare_support.jl"))

@testset "Flight transport backend comparison" begin
    result = transport_compare_test_large_doget()
    @test !isempty(result.metrics)
    @test result.metrics[1].backend == :purehttp2
    if result.grpcserver_supported
        @test !isnothing(result.comparison)
        @test result.comparison.operation == :doget
        @test result.comparison.candidate_backend == :purehttp2
        @test result.comparison.baseline_backend == :grpcserver
    else
        @test isnothing(result.comparison)
        @test occursin("DoGet", result.grpcserver_detail)
    end
end
